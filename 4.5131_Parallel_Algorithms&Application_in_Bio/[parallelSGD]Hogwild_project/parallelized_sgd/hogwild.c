...
static void atomic_decrement(double *dest, double dec_amt) {
	...
	do {
		...
		if (ret_val != orig_val)
			hogwild_num_atomic_dec_collisions++;   //if retrieve an different original value, it indicate the collision has occur
...
int hogwild(thread_array_t iterate, data_t *data, int thread_num) {
	...
	for (int i = 0; i < sparse_sample_grad.len; i++) {
		int index    = sparse_sample_grad.pts[i].index;   		//sparse_sample_grad: a sparse_array_t (data.h)
		double value = sparse_sample_grad.pts[i].value;
		atomic_decrement(&TA_idx(iterate, index, double), get_stepsize()*value);  //TA_idx get the address of the target TA
	}
#ifdef TRACK_GRADIENT_COORDUPDATE
