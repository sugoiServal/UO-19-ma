...
def fetch_CBOW(normalize=True, lower=False, clean_words=False):
    
    path = r'C:\Users\funrr\web_data\embeddings\ettenten.txt.word_vectors'
    return load_embedding(path, format="word2vec", normalize=normalize, lower=lower, clean_words=clean_words) 
...
def fetch_FastText(lang="en", normalize=True, lower=False, clean_words=False):
    ...
    url_vec = 'https://dl.fbaipublicfiles.com/fasttext/vectors-english/wiki-news-300d-1M.vec.zip'

    #path = _fetch_file(url=url_vec, data_dir='embeddings',
    #                   uncompress=False,
    #                   verbose=1)
    path = r'C:\Users\funrr\web_data\embeddings\wiki-news-300d-1M.vec'

    return load_embedding(path, format='word2vec', normalize=normalize, lower=lower, clean_words=clean_words)

def fetch_w2v():
    path = r'C:\Users\funrr\web_data\embeddings\enwiki_20180420_300d.txt'
